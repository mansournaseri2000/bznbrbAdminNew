...
import { Box, Flex, IconButton } from '@/libs/primitives';
  ...
  return (
    <Box width={'160px'} height={'160px'} position={'relative'} style={{ border: `1px dashed ${colorPalette.blue[8]}`, borderRadius: 8 }}>
      <Flex align={'center'} justify={'center'} height={'100%'}>
        <Image src={isOrigin ? `${process.env.NEXT_PUBLIC_BASE_URL_image}${filePath}` : localPath} alt='item image or icon' width={80} height={80} />
      </Flex>
      <ImagePicker2
        resetStore={resetStore}
        localPath={type === 'image' ? 'localImagePath' : 'localIconPath'}
        name={type === 'image' ? 'imagePath' : 'iconPath'}
