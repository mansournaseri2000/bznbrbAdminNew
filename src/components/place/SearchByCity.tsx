...
import { Button, Flex, Grid, Select } from '@/libs/primitives';
              ...
              data?.pages.map((page, index) =>
                data.pages.length === 1 ? (
                  ...
                  <Grid height={'max-content'} align={'start'} key={index} gap={'16px'} mb={'10px'}>
                    {page?.placesDetail?.map((item: { cityName: string; provinceName: string; id: number; name: string; pictures: any }) => (
                      <SearchPlaceCard key={item.id} city={item.cityName} name={item.name} pictures={item.pictures} province={item.provinceName} id={item.id} />
                    ))}
                  </Grid>
                )
              )}
